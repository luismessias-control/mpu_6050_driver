...
def publish_imu(timer_event):
    global init, gyro_x_i, gyro_y_i, gyro_z_i
    global  gyro_x_m, gyro_y_m, gyro_z_m
    ...
    num = 5000
    if init < num:
        print("Percent: ", init*100/num,"%")
        gyro_x_i.append(read_word_2c(GYRO_XOUT_H) / 131.0)
        gyro_y_i.append(read_word_2c(GYRO_YOUT_H) / 131.0)
        gyro_z_i.append(read_word_2c(GYRO_ZOUT_H) / 131.0)
        init += 1
    elif init == num:
        print("Fim")
        gyro_x_m = sum(gyro_x_i)/len(gyro_x_i)
        gyro_y_m = sum(gyro_y_i)/len(gyro_y_i)
        gyro_z_m = sum(gyro_z_i)/len(gyro_z_i)
        init += 1
        
    gyro_x = read_word_2c(GYRO_XOUT_H) / 131.0 - gyro_x_m
    gyro_y = read_word_2c(GYRO_YOUT_H) / 131.0 - gyro_y_m
    gyro_z = read_word_2c(GYRO_ZOUT_H) / 131.0 - gyro_z_m
